	...
	var getGender =  function(){
		...
		var moveAC = function(){
			...
			.call(function(){
				...
				if(postACIndex == 2){
					tempScale = scaleY/(1+Math.random()*2)
				}else if(postACIndex == 3){
					tempScale = scaleY/(1+Math.random()*3)
				}
		...
		var release = function(e){
			...
			if(started==2){
				...
				loadResource(peopleFoot2,"peopleFoot2", 210*scaleY,667*scaleY,scaleY,scaleY,true);
				loadResource(peopleLeg_v,"peopleLeg_v",peopleLeg_vX*scaleY+length,peopleLeg_vY*scaleY,-length,scaleY,true)
				loadResource(peopleFoot3,"peopleFoot3", peopleFoot3X*scaleY+length,peopleFoot3Y*scaleY,scaleY,scaleY,true);
				...
				if(totalLegWidth>=gap && totalLegWidth <= gap+postAC.getBounds().width*tempScale){
					...
					//add male body
					loadResource(peopleBody,"peopleBody", MALE_x*scaleY,468*scaleY,scaleY,scaleY,true);
					...
					.to({x:MALE_x*scaleY+length}, 300, createjs.Ease.linear)
					.on("change", function(e){
						var ratio = e.target.position/300
						peopleLeg_v.scaleX = -length*(1-ratio)
						peopleFoot2.x = (MALE_x-18)*scaleY + ratio*length
					})
					function finish(){
						...
						var calLevel = function(score) {
							...
							$.ajax({
								...
								success: function(res) {
									...
									var rankNum = Math.round((1-res.rank / res.total)*100-1)
